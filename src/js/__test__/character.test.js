...
test('if the name is not between 2 and 10 characters should throw error', () => {
  ...
  expect(() => new Character('accuracy')).toThrowError('The name must be between 2 and 10 characters!');
});

test('if the character is not on the list, throws an error', () => {
  expect(() => new Character('accur')).toThrowError('The type must be selected from the list of suggested types!');
});